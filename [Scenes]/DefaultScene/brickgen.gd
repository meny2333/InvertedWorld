...
func _ready():
	...
	brick_motion_root = Node3D.new()
	brick_motion_root.name = "BrickOverallMotion"
	brick_parent.add_child(brick_motion_root)
...
func get_creation_extension_direction() -> Vector3:
	for path: Path3D in path_3d_array:
		...
		var sample_distance: float = minf(maxf(brick_length + gap, 0.01), path_length)
		var start_position: Vector3 = path.to_global(path.curve.sample_baked(0.0))
		var next_position: Vector3 = path.to_global(path.curve.sample_baked(sample_distance))
		return (next_position - start_position).normalized()
...
func spawn_brick_instant(active_path_index: int):
	...
	brick_motion_root.add_child(brick)
