...
const CYLINDER_BEND_SHADER := preload("./cylinder_bend.gdshader")
...
# Angle detection state
var _angle_already_triggered: bool = false

	# TP奇偶模式状态：奇数次TP后激活，偶数次TP后关闭
...
func _ready():
	...
	# Preload shader/material early to reduce stutter on enable().
...
func _warmup_shader():
	...
	for variant in variants:
		...
		temp_mesh.global_position = Vector3(0, -10000, 0)  # 移到视线外
...
func _on_mainline_turn():
	if not engage_flag:
		return
	# Capture the tangent before the flip (used for join overlap).
	_sync_mapping_now()
...
func _on_mainline_new_line():
	...
	if target_transform:
		...
		# Prefer the actual drawn direction into the cut (includes axial component).
...
func _sync_mapping_now() -> void:
	if not mainline or not is_instance_valid(mainline):
		return
	# Ensure target_transform is up to date for signal-driven cuts.
...
func _process(delta):
	...
	# Keep shader time monotonic and advancing even when movement is tiny/stopped.
	...
	# Apply signal-driven cuts after updating mapping and the active segment.
	if _pending_on_sky:
...
func _cut_current_segment_at_target() -> void:
	...
	# Reverse overlap: extend the new segment backward into this one.
	# Keep this segment's end cap to avoid visible holes at the join.
...
# 蓝黑色和蓝色发光的颜色常量
const ODD_TP_ALBEDO_COLOR := Color(0.1, 0.2, 0.4)  # 蓝黑色
const ODD_TP_EMISSION_COLOR := Color(0.2, 0.6, 1.0)  # 亮蓝色
...
func _create_cylinder_line_internal():
	...
	# Start/end positions stay at the join; we extend geometry in the shader.
	...
	if current_tangent_direction.length_squared() > 0.000001:
	...
	# Keep caps to avoid visible holes at joins.
	...
	current_cylinder_line.layers = 1048575
...
func update_cylinder_trail():
	...
	# While the segment is still very short, keep updating the end so it grows
	# smoothly from the corner (avoids a visible "start block" / extra corner).
	...
	if from_start_sq < min_sq:
		...
		if early_mat and early_mat is ShaderMaterial:
			...
			if dd0.length_squared() > 0.000001:
...
func _check_angle_detection() -> void:
	...
	# Check if we've reached or passed the target angle (considering spiral direction)
